...
import { ArrowRight, Lock, Mail, User, Building, AlertCircle, ShieldCheck } from "lucide-react";
...
export function SignupForm() {
  ...
  const handleSSO = (provider: string) => {
    toast.info(`Creating account via ${provider} SSO...`);
    setIsLoading(true);
    setTimeout(() => {
      ...
      toast.success(`Account created with ${provider}! Redirecting...`);
      router.push("/dashboard");
    }, 800);
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {errorMessage && (
        <motion.div
          ...
          className="p-3 bg-red-500/10 border border-red-500/30 rounded-2xl flex items-center gap-3 text-xs text-red-600 font-semibold"
        >
      ...
      <div className="space-y-1">
        ...
        <div className="relative">
          <User className="w-4 h-4 text-[#666] absolute left-4 top-1/2 -translate-y-1/2" />
          <input
            type="text"
            value={name}
            ...
            required
            className="w-full pl-11 pr-4 py-2.5 bg-white border border-[#E8E5DC] rounded-2xl text-sm font-medium text-[#101010] placeholder:text-[#8E8E93] focus:outline-none focus:border-[#4338FF] shadow-sm"
          />
        </div>
      </div>
      ...
      <div className="space-y-1">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="alex@enterprise-logistics.com"
            required
            className="w-full pl-11 pr-4 py-2.5 bg-white border border-[#E8E5DC] rounded-2xl text-sm font-medium text-[#101010] placeholder:text-[#8E8E93] focus:outline-none focus:border-[#4338FF] shadow-sm"
          />
        </div>
      ...
      <div className="space-y-1">
        <label className="text-xs font-mono font-bold text-[#101010] uppercase">
          Company / Fleet Operator Name
        </label>
        <div className="relative">
          <Building className="w-4 h-4 text-[#666] absolute left-4 top-1/2 -translate-y-1/2" />
          <input
            type="text"
            value={company}
      ...
      {/* Terms checkbox */}
      <div className="pt-1">
        <label className="flex items-start gap-2 text-xs text-[#666] cursor-pointer">
          <input
            type="checkbox"
            checked={agreeTerms}
            onChange={(e) => setAgreeTerms(e.target.checked)}
            className="w-4 h-4 rounded border-[#E8E5DC] text-[#4338FF] focus:ring-[#4338FF] mt-0.5"
          />
          <span>
            I agree to iLogiTrack's Terms of Service and Privacy Policy. 14-day free trial included.
          </span>
        </label>
      </div>
      ...
      <motion.button
        ...
        type="submit"
        disabled={isLoading}
        className="w-full py-3.5 bg-[#4338FF] text-white hover:bg-[#3228D9] font-bold rounded-2xl text-sm shadow-[0_12px_28px_rgba(67,56,255,0.25)] flex items-center justify-center gap-2 transition-colors cursor-pointer"
      >
